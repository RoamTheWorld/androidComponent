...
public class CircleFlowIndicator extends View implements FlowIndicator, AnimationListener {
	...
	public CircleFlowIndicator(Context context, AttributeSet attrs) {
		...
		TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.ViewFlow_CircleFlowIndicator);

		// Gets the active circle type, defaulting to "fill"
		int activeType = a.getInt(R.styleable.ViewFlow_CircleFlowIndicator_activeType, STYLE_FILL);

		// int activeDefaultColor = 0xFF7cba61;// 0xFFFFFFFF
		int activeDefaultColor = Color.parseColor("#000000");// 0xFFFFFFFF

		// Get a custom active color if there is one
		int activeColor = a.getColor(R.styleable.ViewFlow_CircleFlowIndicator_activeColor, activeDefaultColor);

		// Gets the inactive circle type, defaulting to "stroke"
		int inactiveType = a.getInt(R.styleable.ViewFlow_CircleFlowIndicator_inactiveType, STYLE_STROKE);

		// int inactiveDefaultColor = 0x44FFFFFF;
		int inactiveDefaultColor = Color.parseColor("#ECF5FF");
		// Get a custom inactive color if there is one
		int inactiveColor = a.getColor(R.styleable.ViewFlow_CircleFlowIndicator_inactiveColor, inactiveDefaultColor);

		// Retrieve the radius
		radius = a.getDimension(R.styleable.ViewFlow_CircleFlowIndicator_vf_radius, 4.0f);
		radius = 8;

		// Retrieve the spacing
		spacing = a.getDimension(R.styleable.ViewFlow_CircleFlowIndicator_spacing, 4.0f);

		// Retrieve the fade out time
		fadeOutTime = a.getInt(R.styleable.ViewFlow_CircleFlowIndicator_fadeOut, 0);

		a.getBoolean(R.styleable.ViewFlow_CircleFlowIndicator_centered, false);
	...
	private int measureWidth(int measureSpec) {
		...
		// Calculate the width according the views count
			...
			result = (int) (getPaddingLeft() + getPaddingRight() + (count * 2 * radius) + (count - 1) * spacing + 1);

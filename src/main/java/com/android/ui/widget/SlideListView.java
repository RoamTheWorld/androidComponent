...
public class SlideListView extends ListView {
	...
	/** 左侧菜单的长度 */
	private int leftLength = 360;
	/** 右侧菜单的长度 */
	private int rightLength = 360;
		...
		case MotionEvent.ACTION_MOVE:
			if (!canMove && slidePosition != AdapterView.INVALID_POSITION && (Math.abs(ev.getX() - downX) > mTouchSlop && Math.abs(ev.getY() - downY) < mTouchSlop)) {
				...
				if (offsetX > 0 && (this.mode == MOD_BOTH || this.mode == MOD_RIGHT)) {
					System.out.println("touch-->" + "move1");
					/* 从右向左滑 */
					canMove = true;
				} else if (offsetX < 0 && (this.mode == MOD_BOTH || this.mode == MOD_LEFT)) {
					/* 从左向右滑 */
					System.out.println("touch-->" + "move2");
					canMove = true;
				} else {
					System.out.println("touch-->" + "move3");
					canMove = false;
				}
	...
	private void scrollByDistanceX() {
		...
		System.out.println("itemView.getScrollX()====" + itemView.getScrollX());
